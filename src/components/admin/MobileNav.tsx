...
export function MobileNav({ username, pendingCount }: { username: string; pendingCount: number }) {
  ...
  return (
    <>
      {/* 顶部迷你导航 - 仅汉堡菜单和标题 */}
      <header
        className="lg:hidden fixed top-0 left-0 right-0 z-30 h-14 flex items-center justify-between px-4 gap-2"
      ...
      {/* 抽屉菜单 */}
      <nav
        className={`lg:hidden fixed left-0 w-72 z-40 transform transition-transform duration-300 flex flex-col ${
      ...
      >
        ...
        <div className="flex flex-col gap-0.5 p-3 overflow-y-auto flex-1">
          {ADMIN_NAV_GROUPS.map((group, gi) => {
            ...
            return (
              <div key={group.key}>
                {gi > 0 && <div className="my-2 mx-1" style={{ borderTop: '1px solid var(--border)' }} />}
                <p className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider" style={{ color: 'var(--text-secondary)', opacity: 0.6 }}>{group.label}</p>
                {items.map(item => {
