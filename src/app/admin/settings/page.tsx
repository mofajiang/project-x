...
export default function SettingsPage() {
  ...
  const saveProfile = async () => {
    ...
    const res = await fetch('/api/admin/profile', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(profile),  // 包含 displayName
    })
    const data = await res.json()
    ...
    <div className="max-w-xl">
      ...
      <div className="rounded-2xl p-6 mb-6 flex flex-col gap-4" style={{ background: 'var(--bg-secondary)' }}>
        ...
        <div className="flex items-center gap-4">
          <div className="relative w-16 h-16 rounded-full overflow-hidden flex-shrink-0 cursor-pointer"
            style={{ background: 'var(--bg-hover)' }}
            onClick={() => avatarInputRef.current?.click()}>
            {profile.avatar ? (
              <Image src={profile.avatar} alt="头像" fill className="object-cover" />
            ) : (
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>支持 JPG、PNG、GIF，建议 200×200</p>
          </div>
        ...
        {/* 显示名称 */}
        <div>
          <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>显示名称（展示用）</label>
          <IMEInput type="text" value={profile.displayName}
            onValueChange={v => setProfile(p => ({ ...p, displayName: v }))}
            placeholder="我的博客"
            className="w-full px-3 py-2 rounded-2xl text-sm outline-none"
            style={{ background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid transparent' }} />
        </div>
        {/* 账号（handle） */}
        <div>
          <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>账号 @handle（用于登录）</label>
          <IMEInput type="text" value={profile.username}
            onValueChange={v => setProfile(p => ({ ...p, username: v }))}
            placeholder="admin"
            className="w-full px-3 py-2 rounded-2xl text-sm outline-none"
            style={{ background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid transparent' }} />
        </div>
        ...
        {/* 个人简介 */}
        <div>
          <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>个人简介</label>
          <IMETextarea value={profile.bio}
            onValueChange={v => setProfile(p => ({ ...p, bio: v }))}
            rows={3} placeholder="一句话介绍自己..."
      ...
      <div className="rounded-2xl p-6 mb-6 flex flex-col gap-4" style={{ background: 'var(--bg-secondary)' }}>
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center flex-shrink-0 overflow-hidden"
            ...
            {siteIcon ? (
              ...
              <img src={siteIcon} alt="网站图标" className="w-full h-full object-contain" />
            ) : (
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>建议 32×32 或 64×64 的 PNG/ICO 文件</p>
            {siteIcon && (
              <button onClick={() => setSiteIcon('')}
                className="text-xs self-start"
                style={{ color: '#F4212E' }}>移除图标</button>
            )}
          </div>
        ...
        {siteIcon && (
          <div className="flex items-center gap-2">
            <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>当前路径：</span>
            <span className="text-xs font-mono px-2 py-0.5 rounded" style={{ background: 'var(--bg)', color: 'var(--accent)' }}>{siteIcon}</span>
          </div>
        )}
        <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>上传后点击底部「保存设置」生效，浏览器标签页图标将自动更新</p>
      </div>
      ...
      <div className="rounded-2xl p-6 mb-6 flex flex-col gap-4" style={{ background: 'var(--bg-secondary)' }}>
        ...
        <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>拖动排序，自定义左侧导航栏显示的菜单项</p>
        <div className="flex flex-col gap-2">
          {navItems.map((item, i) => (
            <div key={i} className="flex items-center gap-2 p-3 rounded-xl" style={{ background: 'var(--bg-hover)' }}>
              {/* 排序按钮 */}
              <div className="flex flex-col gap-0.5">
                <button onClick={() => moveNavItem(i, -1)} disabled={i === 0}
                  className="w-5 h-5 flex items-center justify-center rounded text-xs disabled:opacity-30"
                  style={{ color: 'var(--text-secondary)' }}>↑</button>
                <button onClick={() => moveNavItem(i, 1)} disabled={i === navItems.length - 1}
                  className="w-5 h-5 flex items-center justify-center rounded text-xs disabled:opacity-30"
                  style={{ color: 'var(--text-secondary)' }}>↓</button>
              </div>
              {/* 图标选择 */}
              <select value={item.icon} onChange={e => updateNavItem(i, 'icon', e.target.value)}
                className="px-2 py-1.5 rounded-lg text-sm outline-none"
                style={{ background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid transparent', minWidth: 90 }}>
                {ICON_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
              {/* 标签 */}
              <IMEInput value={item.label} onValueChange={v => updateNavItem(i, 'label', v)}
                placeholder="标签" className="flex-1 px-2 py-1.5 rounded-lg text-sm outline-none"
                style={{ background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid transparent', minWidth: 60 }} />
              {/* 链接 */}
              <IMEInput value={item.href} onValueChange={v => updateNavItem(i, 'href', v)}
                placeholder="链接 /about" className="flex-1 px-2 py-1.5 rounded-lg text-sm outline-none"
                style={{ background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid transparent', minWidth: 80 }} />
              {/* 删除 */}
      ...
      <div className="rounded-2xl p-6 mb-6 flex flex-col gap-4" style={{ background: 'var(--bg-secondary)' }}>
        ...
        <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>拖动排序，开关控制显示，支持自定义文本组件</p>
        <div className="flex flex-col gap-2">
          {widgets.map((w, i) => (
            <div key={i} className="rounded-xl p-3 flex flex-col gap-2" style={{ background: 'var(--bg-hover)' }}>
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col gap-0.5 flex-shrink-0">
                  <button onClick={() => { const a = [...widgets]; if (i > 0) { [a[i-1],a[i]]=[a[i],a[i-1]]; setWidgets(a) } }}
                    disabled={i === 0} className="w-5 h-5 flex items-center justify-center text-xs disabled:opacity-30"
                    style={{ color: 'var(--text-secondary)' }}>↑</button>
                  <button onClick={() => { const a = [...widgets]; if (i < a.length-1) { [a[i],a[i+1]]=[a[i+1],a[i]]; setWidgets(a) } }}
                    disabled={i === widgets.length - 1} className="w-5 h-5 flex items-center justify-center text-xs disabled:opacity-30"
                    style={{ color: 'var(--text-secondary)' }}>↓</button>
                </div>
                {/* 开关 */}
                <input type="checkbox" checked={w.enabled}
                  onChange={e => setWidgets(v => v.map((x,j) => j===i ? {...x, enabled: e.target.checked} : x))}
                  className="w-4 h-4 flex-shrink-0" />
                {/* 类型选择 */}
                <select value={w.type}
                  onChange={e => setWidgets(v => v.map((x,j) => j===i ? {...x, type: e.target.value as WidgetType} : x))}
                ...
                {/* 删除 */}
                <button onClick={() => setWidgets(v => v.filter((_,j) => j !== i))}
              ...
              <IMEInput
                ...
                placeholder={`标题（留空用默认：${WIDGET_LABELS[w.type]}）`}
              ...
              {w.type === 'custom' && (
                <IMETextarea
                  value={w.content || ''}
                  onValueChange={v => setWidgets(arr => arr.map((x,j) => j===i ? {...x, content: v} : x))}
                  placeholder="自定义内容（支持换行）"
                  rows={3}
              ...
              {w.type === 'links' && (
                <div className="flex flex-col gap-2">
                  {(w.links || []).map((link, li) => (
                    <div key={li} className="flex flex-col gap-1.5 p-2 rounded-lg" style={{ background: 'var(--bg-hover)' }}>
                      <div className="flex gap-2">
                        <IMEInput value={link.label} onValueChange={v => setWidgets(arr => arr.map((x,j) => j!==i ? x : {...x, links: (x.links||[]).map((l,k) => k===li ? {...l, label: v} : l)}))}
                          placeholder="名称" className="flex-1 px-2 py-1 rounded-lg text-sm outline-none"
                          style={{ background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid transparent' }} />
                        <IMEInput value={link.url} onValueChange={v => setWidgets(arr => arr.map((x,j) => j!==i ? x : {...x, links: (x.links||[]).map((l,k) => k===li ? {...l, url: v} : l)}))}
                          placeholder="https://" className="flex-1 px-2 py-1 rounded-lg text-sm outline-none"
                          style={{ background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid transparent' }} />
                        <button onClick={() => setWidgets(arr => arr.map((x,j) => j!==i ? x : {...x, links: (x.links||[]).filter((_,k) => k!==li)}))}
                          className="w-6 h-6 flex items-center justify-center rounded-full flex-shrink-0 text-xs"
                          style={{ color: '#F4212E' }}
                          onMouseEnter={e => (e.currentTarget.style.background = 'rgba(244,33,46,0.1)')}
                          onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>×</button>
                      </div>
                      <div className="flex gap-2">
                        <IMEInput value={link.desc || ''} onValueChange={v => setWidgets(arr => arr.map((x,j) => j!==i ? x : {...x, links: (x.links||[]).map((l,k) => k===li ? {...l, desc: v} : l)}))}
                          placeholder="简介（可选）" className="flex-1 px-2 py-1 rounded-lg text-sm outline-none"
                          style={{ background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid transparent' }} />
                        <IMEInput value={link.avatar || ''} onValueChange={v => setWidgets(arr => arr.map((x,j) => j!==i ? x : {...x, links: (x.links||[]).map((l,k) => k===li ? {...l, avatar: v} : l)}))}
                          placeholder="头像 URL（可选）" className="flex-1 px-2 py-1 rounded-lg text-sm outline-none"
                          style={{ background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid transparent' }} />
                      </div>
                    </div>
                  ))}
                  <button onClick={() => setWidgets(arr => arr.map((x,j) => j!==i ? x : {...x, links: [...(x.links||[]), {label:'', url:'', desc:'', avatar:''}]}))}
                    className="self-start px-3 py-1 rounded-full text-xs font-medium"
                    style={{ background: 'rgba(29,155,240,0.15)', color: 'var(--accent)' }}>+ 添加链接</button>
                </div>
              )}
            </div>
          ))}
      ...
      <div className="rounded-2xl p-6 mb-6 flex flex-col gap-4" style={{ background: 'var(--bg-secondary)' }}>
        ...
        <div>
          <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>博客简介</label>
          <IMETextarea value={config.siteDesc} onValueChange={v => setConfig(c => ({ ...c, siteDesc: v }))} rows={3}
            placeholder="一句话介绍自己..."
            className="w-full px-3 py-2 rounded-2xl text-sm outline-none resize-none"
            style={{ background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid transparent' }} />
        </div>
      ...
      <div className="rounded-2xl p-6 mb-6" style={{ background: 'var(--bg-secondary)' }}>
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          <input type="checkbox" checked={config.commentApproval} onChange={e => setConfig(c => ({ ...c, commentApproval: e.target.checked }))}
            className="w-4 h-4" />
          <div>
            <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>评论需审核后显示</p>
            <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>关闭后评论立即公开显示</p>
          </div>
        </label>
      </div>
      ...
      <div className="rounded-2xl p-6 mb-6 flex flex-col gap-4" style={{ background: 'var(--bg-secondary)' }}>
        ...
        <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>显示在右侧栏底部，支持 HTML（如 &lt;a&gt; 链接）</p>
        <IMETextarea
          value={config.copyright}
          onValueChange={v => setConfig(c => ({ ...c, copyright: v }))}
          rows={3}
          placeholder="© 2024 我的博客 · All rights reserved"
          className="w-full px-3 py-2 rounded-2xl text-sm outline-none resize-none font-mono"
          style={{ background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid transparent' }}
        />
      </div>
      ...
      <div className="rounded-2xl p-6 mb-6" style={{ background: 'var(--bg-secondary)' }}>
        ...
        <p className="text-xs mb-4" style={{ color: 'var(--text-secondary)' }}>访客首次访问时使用的主题，已设置过偏好的用户以本地记录为准</p>
        <div className="flex gap-3">
          <button
            onClick={() => setDefaultTheme('dark')}
            className="flex-1 flex items-center gap-3 px-4 py-3 rounded-2xl border-2 transition-colors"
            style={{
              background: defaultTheme === 'dark' ? 'var(--bg-hover)' : 'transparent',
              borderColor: defaultTheme === 'dark' ? 'var(--accent)' : 'var(--border)',
            }}>
            <span className="text-xl">🌙</span>
            <div className="text-left">
              <p className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>深色模式</p>
              <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>黑色背景，护眼</p>
            </div>
            {defaultTheme === 'dark' && <span className="ml-auto text-xs font-bold" style={{ color: 'var(--accent)' }}>✓ 当前</span>}
          </button>
          <button
            onClick={() => setDefaultTheme('light')}
            className="flex-1 flex items-center gap-3 px-4 py-3 rounded-2xl border-2 transition-colors"
            style={{
              background: defaultTheme === 'light' ? 'var(--bg-hover)' : 'transparent',
              borderColor: defaultTheme === 'light' ? 'var(--accent)' : 'var(--border)',
            }}>
            <span className="text-xl">☀️</span>
            <div className="text-left">
              <p className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>浅色模式</p>
              <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>白色背景，清爽</p>
            </div>
            {defaultTheme === 'light' && <span className="ml-auto text-xs font-bold" style={{ color: 'var(--accent)' }}>✓ 当前</span>}
          </button>
        </div>
      </div>
