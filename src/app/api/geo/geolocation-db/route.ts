...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn('[geo/geolocation-db] upstream error:', response.status, response.statusText)
      return NextResponse.json({ error: 'geolocation-db service unavailable' }, { status: 502 })
    }

    const data = await response.json()
    console.log('[geo/geolocation-db] ip:', ip, 'response:', JSON.stringify(data))
    return NextResponse.json(data)
  } catch (err) {
    console.error('[geo/geolocation-db] request failed:', err instanceof Error ? err.message : String(err))
